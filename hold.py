...
def query_vger(hold, firstitem=0, lastitem=0):
	...
	query = """SELECT BIB_TEXT.LANGUAGE, 
		ITEM_STATUS.ITEM_ID, BIB_TEXT.BIB_ID,
		REGEXP_REPLACE(BIB_TEXT.ISBN,'\s.*','') AS ISBN,
		BIB_TEXT.TITLE_BRIEF,
		LOCATION.LOCATION_NAME,
		ITEM.CREATE_DATE %s%s%s 
		FROM 
		%sBIB_TEXT
		JOIN BIB_MASTER ON BIB_TEXT.BIB_ID = BIB_MASTER.BIB_ID 
		JOIN BIB_MFHD ON BIB_TEXT.BIB_ID = BIB_MFHD.BIB_ID %s
		JOIN MFHD_MASTER ON BIB_MFHD.MFHD_ID = MFHD_MASTER.MFHD_ID 
		JOIN MFHD_ITEM ON MFHD_MASTER.MFHD_ID = MFHD_ITEM.MFHD_ID
		JOIN ITEM ON MFHD_ITEM.ITEM_ID = ITEM.ITEM_ID 
		JOIN ITEM_STATUS ON MFHD_ITEM.ITEM_ID = ITEM_STATUS.ITEM_ID 
				...
				if isbn is not None and isbn != '' and (not re.search('[a-zA-Z:]',isbn) or re.search('[xX]?$',isbn)): # sometimes text has been mistakenly entered into 020$a
					...
					if r.status_code == 200:
						...
						if not tree.xpath("/diagnostics/text()"): # ...because when a record isn't found, the response code is still 200
							...
							field6xx_all = tree.xpath("//marcxml:datafield/@tag[starts-with(.,'6')]",namespaces=NS)
							field6xx_ind2_all = tree.xpath("//marcxml:datafield[@tag[starts-with(.,'6')]]/@ind2",namespaces=NS)
							#600, 610, 611, 65[^3] - subjects
							field6xx = any(x in ['600', '610', '611','650','651','654','655','656','677','658'] for x in field6xx_all)
							field6xx_ind2 = any(x in ['0'] for x in field6xx_ind2_all)
							elvi = ldr[0][17]
							bib_format = ldr[0][6]
							lit = field008[0][33]
...
.sparkcircle {
  ...
  padding-top: 70px; 
	...
	<p><a href="./data/latin.csv">Latin</a> <span id="spark_latin"></span></p>
...
d3.csv('./summaries/latin.csv', function(error, data) {
  sparkline('#spark_latin', data);
});
